...
namespace WorkForceGovProject.Services.ProgramManager
{
    ...
    public class ProgramManagerService : IProgramManagerService
    {
        ...
        // 🚨 NEW: Added method to get ALL benefits
        public async Task<IEnumerable<Benefit>> GetAllBenefitsAsync() =>
            await _pmRepo.GetAllBenefitsAsync();
            ...
            INotificationService notificationService)
        {
            try
            {
                ...
                if (benefit == null)
                    return (false, "Benefit record not found.");
        ...
        public async Task<ProgramManagerDashboardViewModel> GetEnrichedDashboardAsync(int userId)
        {
            try
            {
                ...
                return new ProgramManagerDashboardViewModel
                {
                    ...
                    Notifications = new List<Notification>() // Can be populated via INotificationService later if needed
                };
            }
            catch (Exception ex)
            {
